...
*
********************/
public class PartStatusImpl extends Utils {
	...
	********************/
	//  constructor
	public PartStatusImpl(String sFilename) {
	...
	********************/
	...
	public void addRecToVectorAndSeqFile(
		...
		PartStatus myStatus = new PartStatus();
	...
	********************/
	...
	public void addRecToVectorAndSeqFile(PartStatus istatus) {
		...
		try {
			...
			//istatus.printStatus();
			vList.addElement(new PartStatus(istatus));
		} catch (IOException e) {
			System.err.println(
	...
	********************/
	...
	public int getDamageOnPart(String TaskForceTag, String sname) {
		...
		for (int xx = 0; xx < vList.size(); xx++) {
			PartStatus ss = (PartStatus) vList.elementAt(xx);
	...
	********************/
	...
	public PartStatus getStatusFromShortname(String sname) {

		for (int x = 0; x < vList.size(); x++) {
			PartStatus c = (PartStatus) vList.elementAt(x);
	...
	********************/
	...
	public static void main(String args[]) {
		...
		PartStatusImpl statusimpl = new PartStatusImpl("e:\\newjabber\\status.dat");
		...
		if (iCreate == 1) {
			...
			PartStatus ci1 = new PartStatus("SHIP0345", "", "PE", 3, 0, 1);

			// sets up internal list
			vList.add(ci1);

			//NAME		ABR	 GROUP		COST BONUS MAX STARTQUAN
			PartStatus ci2 = new PartStatus("SHIP0345", "", "GO", 3, 0, 1);
			...
			try {
				...
				for (;;) {
					...
					try {
						...
						vList.addElement((PartStatus) myStatus);
	...
	public void readInventSeqData(Vector List) {
		...
		try {
			while (input.available() != 0) {
				...
				vList.addElement(new PartStatus(sta));
			}
		} catch (IOException e) {
	...
	public boolean readStatusSeqData() {
		...
		try {
			while (input.available() != 0) {
				...
				vList.addElement(new PartStatus(myStatus));
				myStatus.print();
			}
		} catch (IOException e) {
